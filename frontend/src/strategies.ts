...
export const STRATEGIES: Strategy[] = [
    ...
    {
        id: 'eod_directional',
        name: 'EOD Directional (TQQQ/SQQQ)',
        description: 'Analyzes $IUXX (NASDAQ-100 index) using a 5-factor composite score and trades actual TQQQ (3x bull) or SQQQ (3x bear) ETFs. Rides trends with ATR trailing stops. Score-based position sizing deploys more capital on stronger signals.',
        guide: {
            entry: "Computes a composite score from 5 factors: EMA Trend Alignment (±30), RSI Momentum (±25), MACD Histogram (±25), Price Position (±10), and Volume Conviction (±10). When score exceeds the Entry Threshold → buys TQQQ. When score drops below negative Entry Threshold → buys SQQQ.",
            exit: "Exits via ATR Trailing Stop (protects gains) or Signal Reversal (score crosses against position past Exit Threshold). Positions are held through trends — no forced daily exits.",
            tip: "Use $IUXX as the symbol with Daily timeframe. The strategy fetches actual TQQQ/SQQQ prices from the database for realistic execution. Adjust Entry Threshold higher (50-60) for fewer but higher-conviction trades.",
            keyPoints: [
                "Analyzes $IUXX, trades actual TQQQ/SQQQ prices",
                "5-factor composite scoring system",
                "Score-based position sizing (30%-100% of cash)",
                "Captures both upside (TQQQ) and downside (SQQQ)",
                "ATR trailing stops adapt to volatility",
                "Rides trends — holds as long as signal persists"
            ]
        },
        parameters: [
            {
